...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    # Standard supervised fine-tuning loop for BERT
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            # Move batch to the target device (CPU/GPU)
...
def create_augmented_dataloader(args, dataset):
    ...
    # 1) Take the original training split (raw text + label)
    train_raw = dataset["train"]  # columns: "text", "label"

    # 2) Sample 5,000 random training examples and apply our custom transformation
    #    (we use a fixed seed for reproducibility)
    aug_source = train_raw.shuffle(seed=42).select(range(5000))
    aug_transformed = aug_source.map(
        custom_transform,
        load_from_cache_file=False,  # force re-apply transformation
    )

    # 3) Concatenate original training data with the 5k transformed examples
    #    (we keep all original examples + extra noisy copies)
    combined_raw = datasets.concatenate_datasets([train_raw, aug_transformed])

    # 4) Tokenize the combined dataset using the same tokenizer & preprocessing as before
    combined_tokenized = combined_raw.map(
        tokenize_function,
        batched=True,
        load_from_cache_file=False,
    )

    # 5) Prepare for PyTorch: remove text column, rename labels, set format
    ...
    # 6) Build DataLoader (shuffled) and return
